using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class MyGlobals 
{
    public const float CAMERA_ORTHO_SIZE = 50f;
    public const float DESTROY_X_POSITION = -120f;
    public const float SPAWN_X_POSITION = 120f;
    // POSITION INTERVAL OF ALL THE FLOATING OBJECTS
    public const float SURFACE_POSITION = 30f;
    public const float MAX_HEIGHT_GROUND = -8f;
    // REEF 
    public const float REEF_DIMENSION = 12f;
    // QUESTION
    public const float DURATION_EASY_QUESTION = 10f;
    public const float DURATION_HARD_QUESTION = 20f;
    public const int POINTS_EASY_QUESTION = 5;
    public const int POINTS_HARD_QUESTION = 10;
    public const int POINTS_FOR_PASSED_PIPES = 2;
    // Variation of speed with SPEED RING
    public const float SPEED_RING_BOOST = 5f;
    public const float SPEED_OBSTACLE_REDUCTION = 5f;
    //
    public const int WINNING_THRESHOLD = 100;
}
